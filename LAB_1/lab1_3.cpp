 ...
 *
 * Выделяет память под массив из указанного количества элементов.
 ...
 *
 * Перед обращением к элементу проверяется, находится ли индекс
 * в допустимых пределах массива. Если индекс выходит за границы,
 * выводится сообщение об ошибке и возвращается ссылка на статическую
 * переменную-заглушку.
 *
 ...
 *
 * Функция не изменяет массив, поэтому используется константная
 * ссылка на структуру.
 *
 ...
 *
 * Создаёт новый массив заданного размера и копирует в него
 * элементы из старого массива.
 *
 * Если новый размер меньше текущего, элементы, которые не помещаются
 * в новый массив, выводятся на экран.
 *
 * Если новый размер больше текущего, новые элементы автоматически
